...
def jogo_view(page: ft.Page):
    ...
    # para teste =================
    def testar_fim_de_jogo(e):
        jogador_id = page.client_storage.get("jogador_id")
        sala_id = page.client_storage.get("sala_jogador")
        sala_ref = firestore.client().collection("salas").document(sala_id)

        meu_path = page.client_storage.get("meu_caminho")
        oponente_path = "player2" if meu_path == "player1" else "player1"

        sala_ref.update({
            f"{meu_path}.placar.total_geral": 4200,
            f"{oponente_path}.placar.total_geral": 4200
        })

        page.snack_bar = ft.SnackBar(ft.Text("🎯 Pontuação de teste aplicada. Jogue normalmente para simular vitória."))
    ...
    def on_snapshot(doc_snapshot, changes, read_time):
        ...
        for doc in doc_snapshot:
            ...
            # 🧩 Ignora se o jogo terminou
            # O 'return' foi removido para permitir que a lógica de finalização no final da função seja executada.
            # if data.get("game_status") == "finished":
                # print("🛑 Snapshot recebido, mas jogo finalizado. Permitindo finalização e redirecionamento.")
                # NENHUM 'return' AQUI
